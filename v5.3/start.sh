if [ ! -d gitRepo ]; then
    ...
    # git clone
    str="${GIT_REPO}"
    export CHECK_ADDR=$(echo ${str:0:5})
	...
	echo "${CHECK_ADDR}"
        if [ "${CHECK_ADDR}" == "https" ]; then
            ...
            export PRIVATE_REPO=$(echo ${GIT_REPO} | sed "s#http://#http://${GITHUB_ID}:${USR_SECRET}@#g")
            sed -i "s#{GIT_REPO}#${PRIVATE_REPO}#g" /root/usr_build_cmd.sh  
	fi
    else
        sed -i 's/{"git-token":"//g' /secret/usr_secret
        ...
        if [ "${CHECK_ADDR}" == "https" ]; then
            ...
            export PRIVATE_REPO=$(echo ${GIT_REPO} | sed "s#http://#http://${GITLAB_TOKEN_NAME}:${USR_SECRET}@#g")
        fi
        sed -i "s#{GIT_REPO}#${PRIVATE_REPO}#g" /root/usr_build_cmd.sh 
    fi
